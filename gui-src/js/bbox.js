    ...
    try {
      ...
      if (next) {
        ...
        throw {
          ...
          message:
            "The data is not a recognized format:\n \
...
4. WKT\n\n ",
    ...
    if (!fail) {
      this._formatHandler[this.parse_type].call(
        this._formatHandler,
        parsed_data
      );
    }
...
$(document).ready(function () {
  ...
  $('input[type="textarea"]').on("click", function (evt) {
    this.select();
  });
  ...
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    attribution:
      '&copy; Map data from <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>',
    maxZoom: 19,
  }).addTo(map);

  // GSI ハザードマップ (洪水浸水想定区域)
  L.tileLayer(
    "https://disaportaldata.gsi.go.jp/raster/01_flood_l2_shinsuishin/{z}/{x}/{y}.png",
    {
      opacity: 0.5,
      attribution: "国土地理院",
    }
  ...
  map.on("draw:created", function (e) {
    ...
    if (
      ...
      !(
        ...
        drawnItems.getLayers()[0] instanceof L.Marker
      )
    ) {
